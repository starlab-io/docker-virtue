...
with open('/etc/virtue-id', 'r') as f:
    virtue_id = f.read().strip()

with open('/etc/application-id', 'r') as f:
    application_id = f.read().strip()

elasticLog = logging.getLogger('elasticLog')
elasticHandler = CMRESHandler(
    hosts=[{'host': 'aggregator.galahad.com', 'port': 9200}],
    auth_type=CMRESHandler.AuthType.HTTPS,
    es_index_name='appsensor',
    use_ssl=True,
    verify_ssl=False,
    buffer_size=2,
    ...
    es_additional_fields={'virtue_id': virtue_id,
...
log = logging.getLogger('appsensor')
logfile = logging.FileHandler('/opt/appsensor/appsensor.log')
...
def main():
    ...
    log.info('Started App Sensor')

    # Wait for changes to config.json to update thread collection rates
    while True:
        with open('/opt/appsensor/config.json', 'r') as f:
            # Acquire file lock
            while True:
                try:
